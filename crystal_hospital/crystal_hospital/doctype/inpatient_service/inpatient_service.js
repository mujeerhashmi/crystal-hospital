...
frappe.ui.form.on('InPatient Service', {
	...
	inpatient_service: function(frm) {
...
let set_item_details = function(frm) {
	if (frm.doc.inpatient_service && frm.doc.is_billable) {
		if (!frm.doc.item_code)
			frm.set_value('item_code', frm.doc.inpatient_service);
		if (!frm.doc.description)
			frm.set_value('description', frm.doc.inpatient_service);
...
let change_item_code = function(frm, doc) {
	let d = new frappe.ui.Dialog({
		...
		primary_action: function() {
			...
			if (values) {
				frappe.call({
					"method": "erpnext.healthcare.doctype.inpatient_service.inpatient_service.change_item_code",
