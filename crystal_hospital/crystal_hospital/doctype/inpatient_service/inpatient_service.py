...
class InPatientService(Document):
	def validate(self):		
		if self.is_billable:
			if self.disabled:
	...
	def on_trash(self):
		if self.item:
			try:
				...
				frappe.throw(_('Not permitted. Please disable the InPatient Service'))
...
def create_item(doc):
	...
	item =  frappe.get_doc({
		...
		'item_name': doc.inpatient_service,
...
def change_item_code(item, item_code, doc_name):
	if frappe.db.exists({'doctype': 'Item', 'item_code': item_code}):
		...
		frappe.db.set_value('InPatient Service', doc_name, 'item_code', item_code)
